...
import { ISupportTicket, SupportTicketCategory } from '@bushtrade/administration-portal/shared/entites';

export const SUPPORTTICKETS_FEATURE_KEY = 'supportTickets';

export interface State extends EntityState<ISupportTicket> {
  selectedId?: string; // which SupportTickets record has been selected
...
const supportTicketsReducer = createReducer(
  ...
  on(SupportTicketsActions.closeSupportTicket, (state) => ({
    ...state,
    error: null,
  })),
  on(
    ...
    (state, action) =>{
      return supportTicketsAdapter.updateOne({ id: state.selectedId, changes: action.supportTicket }, { ...state })
    }
  ),
  on(SupportTicketsActions.closeSupportTicketFailure, (state, { error }) => ({
    ...state,
    error,
  }))
);
